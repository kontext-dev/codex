...
fn load_csv_dataset(path: &Path) -> Result<Vec<McpAtlasTask>> {
    ...
    for (row_num, line_result) in lines.enumerate() {
        ...
        let enabled_tools = parse_json_string_array(&fields[tools_idx]).with_context(|| {
            format!(
                "Failed to parse ENABLED_TOOLS for task {} (row {})",
                task_id,
                row_num + 2
            )
        })?;
        let prompt = fields[prompt_idx].clone();
        let claims = parse_json_string_array(&fields[claims_idx]).with_context(|| {
            format!(
                "Failed to parse GTFA_CLAIMS for task {} (row {})",
                task_id,
                row_num + 2
            )
        })?;
        let trajectory = parse_trajectory(&fields[trajectory_idx]).with_context(|| {
            format!(
                "Failed to parse TRAJECTORY for task {} (row {})",
                task_id,
                row_num + 2
            )
        })?;
...
fn load_arrow_dataset(path: &Path) -> Result<Vec<McpAtlasTask>> {
    ...
    let reader = StreamReader::try_new(buf_reader, None)
        .with_context(|| "Failed to create Arrow stream reader")?;
    ...
    for batch_result in reader {
        ...
        for i in 0..batch.num_rows() {
            ...
            let enabled_tools =
                parse_json_string_array(tools_array.value(i)).with_context(|| {
                    format!("Failed to parse ENABLED_TOOLS for task {task_id} (row {i})")
                })?;
            let prompt = prompt_array.value(i).to_string();
            let claims = parse_json_string_array(claims_array.value(i)).with_context(|| {
                format!("Failed to parse GTFA_CLAIMS for task {task_id} (row {i})")
            })?;
            let trajectory = parse_trajectory(trajectory_array.value(i)).with_context(|| {
                format!("Failed to parse TRAJECTORY for task {task_id} (row {i})")
            })?;
...
mod tests {
    ...
    fn test_parse_trajectory() {
        let json =
            r#"[{"tool": "git", "args": {"command": "status"}}, {"tool": "cli", "args": {}}]"#;
