...
impl RlmController {
    ...
    pub async fn from_state(config: RlmConfig, state: RlmSessionState) -> Result<Self, RlmError> {
        ...
        if let Some(manifest) = state.corpus_manifest {
            ...
            let corpus_root = PathBuf::from(format!("/tmp/rlm-corpus-{}", state.session_id));
    ...
    pub async fn get_chunk(
        ...
        let evidence = EvidenceItem::new(
            ...
            EvidenceSource::from_file(
                chunk_id,
                Some(content.offset),
                Some(content.original_length),
            ),
        );
...
"#,
            ...
            "**CORPUS:** Not yet initialized. Use rlm_ingest_prompt to store large prompts."
                .to_string()
        };
...
mod tests {
    ...
    async fn test_gateway_system_prompt() {
        ...
        let prompt = controller
            .build_gateway_system_prompt(evidence_summary)
            .await;
