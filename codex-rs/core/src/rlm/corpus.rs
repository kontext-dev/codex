...
impl RlmCorpus {
    ...
    pub async fn ingest_prompt(&self, prompt: &str) -> Result<CorpusManifest, RlmError> {
        ...
        while offset < total_bytes {
            ...
            let chunk_id = format!("c{chunk_idx:04}");
            let chunk_file = PathBuf::from(format!("chunk_{chunk_id}.txt"));
    ...
    pub async fn append_content(
        ...
        while offset < content_bytes {
            ...
            let chunk_id = format!("{append_prefix}_{chunk_idx:04}");
            let chunk_file = PathBuf::from(format!("chunk_{chunk_id}.txt"));
            let chunk_path = self.root.join(&chunk_file);
    ...
    pub async fn rebuild_index(&self) -> Result<(), RlmError> {
        ...
        for chunk_info in &manifest.chunks {
            ...
            if content.len() < 10000 {
                index.content_cache.insert(chunk_info.id.clone(), content);
            }
        }
...
mod tests {
    ...
    async fn test_append_large_content() {
        ...
        for chunk in &new_chunks {
            assert_eq!(chunk.source_id.as_deref(), Some("tool:list_all:call_456"));
        }
    }
    ...
    async fn test_rebuild_index() {
        ...
        let restored = RlmCorpus::from_manifest(temp_dir.path().to_path_buf(), manifest, config)
            .await
            .unwrap();
