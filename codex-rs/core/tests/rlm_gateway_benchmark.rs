...
async fn benchmark_baseline(scenario: &BenchmarkScenario) -> ModeResult {
    ...
    let quality_score = if total_context_tokens > 128000 {
        0
    } else {
        100
    };
...
async fn benchmark_codemode(scenario: &BenchmarkScenario) -> ModeResult {
    ...
    let total_latency_ms = max_latency + 50; // Add VM overhead
...
async fn benchmark_rlm(scenario: &BenchmarkScenario) -> ModeResult {
    let temp_dir =
        TempDir::new().unwrap_or_else(|e| panic!("failed to create temp dir for benchmark: {e}"));
    ...
    let corpus = RlmCorpus::new(temp_dir.path().to_path_buf(), config.clone())
        .await
        .unwrap_or_else(|e| panic!("failed to initialize RLM corpus: {e}"));
    corpus
        .ingest_prompt("Initial corpus.")
        .await
        .unwrap_or_else(|e| panic!("failed to ingest benchmark prompt: {e}"));
    ...
    for (i, tool_name) in scenario.tools.iter().enumerate() {
        if let Some(tool) = SIMULATED_TOOL_RESULTS.iter().find(|t| t.name == *tool_name) {
            ...
            let result = router
                .process_result(&format!("call_{i}"), "kontext-dev", tool_name, &content)
                .await
                .unwrap_or_else(|e| {
                    panic!("failed to process benchmark result for {tool_name}: {e}")
                });

            match result {
                ProcessedResult::StoredInCorpus { chunk_ids: _, .. } => {
                    evidence_count += 1;
                }
...
async fn benchmark_evidence_summary_generation() {
    ...
    for i in 0..10 {
        let content = format!("Tool result {i} with some content here.");
        router
